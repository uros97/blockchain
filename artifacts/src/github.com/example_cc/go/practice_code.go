...
func (t *SimpleChaincode) Init(stub shim.ChaincodeStubInterface) pb.Response {
	...
	var kredit1 = kredit{"kr1", "1.1.2014.", "1.1.2016.", 100.0, 2.5, 24, 24, 2400.0}
	var kredit2 = kredit{"kr2", "2.1.2014.", "2.1.2016.", 100.0, 2.5, 24, 20, 2400.0}
	var kredit3 = kredit{"kr3", "3.1.2014.", "3.1.2016.", 100.0, 2.5, 24, 24, 2400.0}
	var kredit4 = kredit{"kr4", "4.1.2014.", "4.1.2016.", 100.0, 2.5, 24, 20, 2400.0}
	kreditId = 5
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if function == "izvrsiTransakciju" {
		return t.izvrsiTransakciju(stub, args)
	}
	...
	if function == "uplataRateKredita" {
		return t.uplataRateKredita(stub, args)
	}
...
func (t *SimpleChaincode) dodajKorisnika(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var ime, prezime, email, kolicinaNovca string // Entities
	...
	var newKorisnik = korisnik{korId, korId, ime, prezime, email, kolicinaNovcaFloat, newKrediti, newTransakcije}
...
func (t *SimpleChaincode) podizanjeKredita(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	mesecnaRata := (iznosKreditaFloat + iznosKreditaFloat*kamataFloat*0.01) / float64(brojRataInt)
...
func (t *SimpleChaincode) uplataRateKredita(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if len(args) != 1 {
		return shim.Error("Netacan broj argumenata! Ocekivani argumenti: ID korisnika")
	}
	//load korisnik
	loadedKorisnik, err := stub.GetState(idKorisnika)
	...
	if korisnikFromJson.Krediti[rbrKredita].BrojOtplacenihRate == korisnikFromJson.Krediti[rbrKredita].BrojRata {
		korisnikFromJson.Krediti[rbrKredita].DatumZavrsetka = time.Now().Format("01-02-2006")
	}
	...
	ajson, _ = json.Marshal(korisnikFromJson.Krediti[rbrKredita])
	err = stub.PutState(korisnikFromJson.Krediti[rbrKredita].ID, ajson)
	if err != nil {
		return shim.Error(err.Error())
	}
...
func (t *SimpleChaincode) izvrsiTransakciju(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	var newTransakcijaId = "t" + strconv.Itoa(transakcijaId)
	transakcijaId++
	var newTransakcija = transakcija{newTransakcijaId, time.Now().Format("01-02-2006"), uplatilacFromJson.ID, primaocFromJson.ID, iznosTransakcijeFloat}

	uplatilacFromJson.Transakcije = append(uplatilacFromJson.Transakcije, newTransakcija)
	primaocFromJson.Transakcije = append(primaocFromJson.Transakcije, newTransakcija)

	ajson, _ := json.Marshal(newTransakcija)
	err = stub.PutState(newTransakcija.ID, ajson)
	if err != nil {
		return shim.Error(err.Error())
	}
